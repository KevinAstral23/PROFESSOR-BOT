...
async def pm_next_page(bot, query):
    ...
    if not search:
        await query.answer("𝖺 𝖯𝖬 𝗂𝗌𝗌𝗎𝖾 𝗍𝗋𝗒 𝖺𝗀𝖺𝗂𝗇 𝗈𝗇 𝖺 𝗀𝗋𝗈𝗎𝗉.", show_alert=True)
        return
...
async def pm_spoll_tester(bot, query):
    ...
    await query.answer('𝖧𝖮𝖫𝖣 𝖴𝗉 𝗅𝖾𝗍 𝗆𝖾 𝗅𝗈𝗈𝗄...')
    files, offset, total_results = await get_search_results(movie, offset=0, filter=True)
    if files:
        k = (movie, files, offset, total_results)
        await pm_AutoFilter(bot, query, k)
    else:
        k = await query.message.edit('𝖭𝖮𝖳 𝗈𝗇 𝗆𝗒 𝖽𝖺𝗍𝖺𝖻𝖺𝗌𝖾 𝖨𝗅𝗅 𝗇𝗈𝗍𝗂𝖿𝗒 𝗆𝗒 𝖺𝖽𝗆𝗂𝗇𝗌')
...
async def pm_AutoFilter(client, msg, pmspoll=False):  
    ...
    if imdb:
        ...
        cap = f"<𝖻>𝖧𝖾𝗋𝖾'𝗌 𝗐𝗁𝖺𝗍 𝖨 𝖿𝗈𝗎𝗇𝖽{search}</𝖻>"
...
async def pm_spoll_choker(msg):
    ...
    gs_parsed = []
    if not g_s:
        k = await msg.reply("<b>𝖢𝖺𝗇𝗍 𝖿𝗂𝗇𝖽 𝗂𝗍 𝖨𝗅𝗅 𝗇𝗈𝗍𝗂𝖿𝗒 𝗆𝗒 𝖺𝖽𝗆𝗂𝗇𝗌..</b>.")
    ...
    if not movielist:
        k = await msg.reply("𝖨 𝖼𝖺𝗇𝗍 𝖿𝗂𝗇𝖽 𝗂𝗍, 𝖼𝗁𝖾𝖼𝗄 𝗒𝗈𝗎𝗋 𝗌𝗉𝖾𝗅𝗅𝗂𝗇𝗀...")
        await asyncio.sleep(8)
        await k.delete()
        return
    ...
    await msg.reply("𝖨 𝖼𝖺𝗇𝗍 𝖿𝗂𝗇𝖽 𝗂𝗍, 𝖼𝗁𝖾𝖼𝗄 𝗒𝗈𝗎𝗋 𝗌𝗉𝖾𝗅𝗅𝗂𝗇𝗀...\n𝖣𝗂𝖽 𝖸𝖮𝖴 𝗆𝖾𝖺𝗇..?", reply_markup=InlineKeyboardMarkup(btn), reply_to_message_id=msg.id)
