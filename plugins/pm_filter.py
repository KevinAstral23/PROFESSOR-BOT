...
async def pm_next_page(bot, query):
    ...
    if n_offset == 0:
        btn.append(
            [InlineKeyboardButton("◀️ 𝖡𝖺𝖼𝗄", callback_data=f"pmnext_{req}_{key}_{off_set}"),
             InlineKeyboardButton(f"📃 𝖯𝖺𝗀𝖾𝗌 {math.ceil(int(offset) / 10) + 1} / {math.ceil(total / 10)}", callback_data="pages")]                                  
        )
        ...
        btn.append(
            [
                InlineKeyboardButton("◀️ 𝖡𝖺𝖼𝗄", callback_data=f"pmnext_{req}_{key}_{off_set}"),
                InlineKeyboardButton(f"🗓 {math.ceil(int(offset) / 10) + 1} / {math.ceil(total / 10)}", callback_data="pages"),
                InlineKeyboardButton("𝖭𝖾𝗑𝗍 ▶️", callback_data=f"pmnext_{req}_{key}_{n_offset}")
            ],
        )
    try:
...
async def pm_AutoFilter(client, msg, pmspoll=False):    
    ...
    if offset != "":
        ...
        btn.append(
            ...
            InlineKeyboardButton(text="𝖭𝖾𝗑𝗍 ▶️", callback_data=f"pmnext_{req}_{key}_{offset}")]
        )
    else:
        btn.append(
...
    photo="https://telegra.ph/file/995e382f36f89e8efb93a.jpg",
