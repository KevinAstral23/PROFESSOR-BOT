...
async def pm_next_page(bot, query):
    ...
    if not search:
        return await query.answer("𝖭𝖮𝖳 𝖮𝗇𝗅𝗒 𝖿𝗈𝗈𝖽 𝖾𝗑𝗉𝗂𝗋𝖾𝗌 𝖻𝗋𝗈 𝖺𝗅𝗌𝗈 𝗍𝗁𝗂𝗌 𝖻𝗎𝗍𝗍𝗈𝗇 𝗒𝗈𝗎𝗋𝖾 𝗍𝗋𝗒𝗇𝖺 𝖼𝗅𝗂𝖼𝗄, 𝗋𝖾𝗊𝗎𝖾𝗌𝗍 𝖺𝗀𝖺𝗂𝗇.", show_alert=True)
...
async def pm_spoll_tester(bot, query):
    ...
    if files:
        ...
        k = await query.message.edit('𝖨'𝗅𝗅 𝗍𝖾𝗅𝗅 𝗆𝗒 𝖺𝖽𝗆𝗂𝗇𝗌.. 𝗍𝗁𝖾𝗒'𝗅𝗅 𝖺𝖽𝖽 𝗂𝗍 𝗂𝗇 𝖺 𝖿𝖾𝗐..')
...
async def pm_AutoFilter(client, msg, pmspoll=False):    
    ...
    if offset != "":
        ...
        btn.append(
            [InlineKeyboardButton(text="📄 𝖭𝖤𝖷𝖳 1/1", callback_data="pages")]
        )
    if PM_IMDB:
    ...
    if imdb:
        ...
        cap = f"𝖧𝖾𝗋𝖾'𝗌 𝗐𝗁𝖺𝗍 𝖨 𝗀𝗈𝗍 {search}"
...
async def pm_spoll_choker(msg):
    ...
    gs_parsed = []
    if not g_s:
        k = await msg.reply("𝖨 𝗍𝗋𝗂𝖾𝖽.. 𝖺 𝗆𝗈𝗏𝗂𝖾 𝗂𝗇 𝗍𝗁𝖺𝗍 𝗇𝖺𝗆𝖾 𝖨 𝖼𝖺𝗇𝗍 𝖿𝗂𝗇𝖽 𝗂𝗍.", quote=True)
    ...
    if not movielist:
        k = await msg.reply("𝖨'𝗆 𝗀𝗈𝗇𝗇𝖺 𝗍𝖺𝗄𝖾 𝗒𝗈𝗎 𝖻𝖺𝖼𝗄 𝗍𝗈 𝗄𝗂𝗇𝖽𝖾𝗋𝗀𝖺𝗋𝗍𝖾𝗇 𝗍𝗈 𝗅𝖾𝖺𝗋𝗇 𝖠𝖡𝖢 𝖢𝖧𝖤𝖢𝖪 𝖴𝖱 𝖲𝖯𝖤𝖫𝖫𝖨𝖭𝖦!!", quote=True)
        await asyncio.sleep(8)
        await k.delete()
        return
    ...
    await msg.reply("𝖢𝖺𝗇'𝗍 𝖥𝗂𝗇𝖽 𝖨𝗍 𝖡𝗋𝖺𝗏 𝖣𝗈 𝗒𝗈𝗎 𝗆𝖾𝖺𝗇 𝖺𝗇𝗒 𝗈𝖿 𝗍𝗁𝖾𝗌𝖾?", reply_markup=InlineKeyboardMarkup(btn), quote=True)
